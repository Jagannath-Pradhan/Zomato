...
updateRestaurant = (req, res, next) => {
    ...
    // restaurantSchema.updateMany({ "name": req.params.name }, { "address": req.body.address }, (err, reponse) => {
        if(err)
            res.send("Exception Occurred")
        else
...
module.exports = { getRestaurants, getRestaurantByName, getRestaurantById, addRestaurant, updateRestaurant, deleteRestaurantByName}